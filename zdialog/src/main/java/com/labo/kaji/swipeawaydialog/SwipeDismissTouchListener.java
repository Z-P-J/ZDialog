    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (motionEvent.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                final boolean dismissDown;
                if (Math.abs(deltaY) > mViewHeight / 2f && mSwiping) {
                    dismiss = true;
...
//                            .rotation(mTiltEnabled ? (dismissRight ? 45 : -45) : 0f)
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if ((Math.abs(deltaX) > mSlop && Math.abs(deltaY) < Math.abs(deltaX) / 2)) {
                    ...
                    cancelEvent.recycle();
                    deltaY = 0;
                } else if (Math.abs(deltaY) > mSlop && Math.abs(deltaX) < Math.abs(deltaY) / 2) {
                    mSwiping = true;
...
//                    mView.setAlpha(Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(deltaX) / mViewWidth)));
                    mView.setScaleX(mView.getAlpha());
                    mView.setScaleY(mView.getAlpha());
                    return true;
                }
                break;
            }
